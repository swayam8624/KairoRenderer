...
export namespace kairo::renderer
{
    ...
    class VulkanRendererRuntime final
    {
    public:
        explicit VulkanRendererRuntime(const WindowDesc& windowDesc)
            : m_Glfw(),
              m_Window(windowDesc, GraphicsBackend::Vulkan),
              m_Instance(MakeInstanceDesc(), RequiredExtensions()),
              m_Surface(m_Instance, m_Window),
              m_Device(m_Instance, m_Surface),
              m_Swapchain(m_Device, m_Surface, m_Window),
              m_Command(m_Device),
              m_Sync(m_Device, static_cast<std::uint32_t>(m_Swapchain.Images().size())),
              m_Triangle(m_Device, m_Swapchain),
              m_PickReadback(m_Device, sizeof(std::uint32_t), VK_BUFFER_USAGE_TRANSFER_DST_BIT)
        {
        }
