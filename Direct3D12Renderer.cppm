...
#include "detail/Direct3D12Backend.hpp"
...
export module Kairo.Renderer.Direct3D12Runtime;
...
{
    /// Native Direct3D12 implementation of the renderer-neutral runtime API.
    ///
    ...
    /// DXGI swap-chain presentation surface. Task: keep COM ownership and
    /// Direct3D12 resource types behind `detail::Direct3D12Backend` while preserving the
    /// same public behavior used by Vulkan and OpenGL hosts.
    class Direct3D12RendererRuntime final
    {
        GlfwRuntime m_Glfw;
        Window m_Window;
        detail::Direct3D12Backend m_Backend;
        ...
        std::vector<detail::Direct3D12DebugVertex> m_DebugVertices;
    ...
    public:
        explicit Direct3D12RendererRuntime(const WindowDesc& desc)
            : m_Glfw(), m_Window(desc, GraphicsBackend::Direct3D12),
              m_Backend(m_Window.NativeHandle(), desc.Width, desc.Height),
              m_ViewportWidth(desc.Width), m_ViewportHeight(desc.Height)
        {
        }

        Direct3D12RendererRuntime(const Direct3D12RendererRuntime&) = delete;
        Direct3D12RendererRuntime& operator=(const Direct3D12RendererRuntime&) = delete;
        ...
        /// Task: expose the native Direct3D12 render/present and readback
        ...
        void DrawFrame()
        {
            ...
            RenderGraph graph;
            const auto viewport = graph.AddResource({ "Direct3D12Viewport",
                RenderResourceKind::External, 0u, false,
                RenderResourceState::ShaderRead });
            const auto nativeFrame = graph.AddResource({ "Direct3D12NativeFrame",
                RenderResourceKind::External, 0u, false,
        ...
        void SubmitDebugDraw(const DebugDrawList& debug)
        {
            if (debug.Lines().size() >
                ...
                throw std::length_error("Direct3D12 debug-line count overflows storage.");
        ...
        [[nodiscard]] MeshHandle CreateMesh(const Mesh& mesh)
        {
            ...
            {
                detail::Direct3D12Vertex converted{};
        ...
        void DestroyMesh(MeshHandle mesh)
        {
            if (!m_Meshes.contains(mesh))
                throw std::invalid_argument(
                    "Direct3D12 mesh handle is not owned by this runtime.");
            if (std::ranges::any_of(m_Draws,
                [&](const MeshDraw& draw) { return draw.Mesh == mesh; }))
                throw std::logic_error(
                    "Cannot destroy a Direct3D12 mesh used by the submitted scene.");
            ...
            TextureSampler sampler = {})
        {
            ...
            for (const auto& mip : texture.Mips)
            {
                if (mip.Pixels.size() > std::numeric_limits<std::size_t>::max() - total)
                    throw std::length_error("Direct3D12 texture payload overflows storage.");
                total += mip.Pixels.size();
            }
            bytes.reserve(total);
            for (const auto& mip : texture.Mips)
                bytes.insert(bytes.end(), mip.Pixels.begin(), mip.Pixels.end());
            detail::Direct3D12TextureUpload upload;
        ...
        void DestroyTexture(TextureHandle texture)
        {
            if (!m_Textures.contains(texture))
                throw std::invalid_argument(
                    "Direct3D12 texture handle is not owned by this runtime.");
            if (TextureIsSubmitted(texture))
                throw std::logic_error(
                    "Cannot destroy a Direct3D12 texture used by the submitted scene.");
        ...
        void ResizeViewport(std::uint32_t width, std::uint32_t height)
        {
            ...
            if (width > 32'768u || height > 32'768u)
                throw std::invalid_argument(
                    "Direct3D12 viewport dimensions exceed 32768.");
        ...
        [[nodiscard]] Direct3D12ViewportTexture ViewportTexture() const noexcept
        {
            const auto descriptor = m_Backend.ViewportDescriptor();
            return { { descriptor.CPU, descriptor.GPU }, m_ViewportWidth,
                m_ViewportHeight, m_ViewportGeneration };
        }
        [[nodiscard]] Direct3D12BackendContext BackendContext() const
        {
            Direct3D12BackendContext context;
            context.Device = m_Backend.Device();
            context.CommandQueue = m_Backend.CommandQueue();
            context.ShaderResourceHeap = m_Backend.ShaderResourceHeap();
            context.FramesInFlight = 2u;
            context.RenderTargetFormat = 28u; // DXGI_FORMAT_R8G8B8A8_UNORM
            context.DepthStencilFormat = 40u; // DXGI_FORMAT_D32_FLOAT
            context.AllocateDescriptor = [this]
            {
                const auto value = m_Backend.AllocateToolingDescriptor();
                return Direct3D12Descriptor{ value.CPU, value.GPU };
            };
            context.FreeDescriptor = [this](Direct3D12Descriptor descriptor)
            {
                m_Backend.FreeToolingDescriptor(
    ...
    private:
        ...
        [[nodiscard]] static detail::Direct3D12Draw Convert(const MeshDraw& draw)
        {
            detail::Direct3D12Draw output;
            ...
            output.Material.MetallicRoughnessTexture =
                draw.Material.MetallicRoughnessTexture;
        ...
        [[nodiscard]] static detail::Direct3D12Light Convert(const RenderLight& light)
        {
            detail::Direct3D12Light output;
        ...
        [[nodiscard]] static detail::Direct3D12DebugVertex Convert(
        ...
        [[nodiscard]] static detail::Direct3D12TextureFormat Convert(
            kairo::assets::TexturePixelFormat format,
            kairo::assets::TextureColorSpace colorSpace)
        {
            using F = kairo::assets::TexturePixelFormat;
            if (format == F::R8) return detail::Direct3D12TextureFormat::R8Linear;
            if (format == F::RG8) return detail::Direct3D12TextureFormat::RG8Linear;
            if (format == F::RGBA16Float)
                return detail::Direct3D12TextureFormat::RGBA16Float;
            if (format == F::RGBA8 &&
                colorSpace == kairo::assets::TextureColorSpace::SRGB)
                return detail::Direct3D12TextureFormat::RGBA8SRGB;
            if (format == F::RGBA8) return detail::Direct3D12TextureFormat::RGBA8Linear;
            throw std::invalid_argument("Unsupported Direct3D12 texture format.");
        }

        [[nodiscard]] static detail::Direct3D12AddressMode Convert(
            TextureAddressMode mode)
        {
            switch (mode)
            {
                case TextureAddressMode::Repeat:
                    return detail::Direct3D12AddressMode::Repeat;
                case TextureAddressMode::MirroredRepeat:
                    return detail::Direct3D12AddressMode::Mirror;
                case TextureAddressMode::ClampToEdge:
                    return detail::Direct3D12AddressMode::Clamp;
            }
            throw std::invalid_argument("Unsupported Direct3D12 texture address mode.");
        }
        ...
        void ValidateTexture(TextureHandle texture) const
        {
            if (texture != InvalidTextureHandle && !m_Textures.contains(texture))
                throw std::invalid_argument(
                    "RenderScene references a Direct3D12 texture owned by another runtime.");
        }
